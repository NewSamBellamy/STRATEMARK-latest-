...
/**
 * In-memory session profile: when storage is unavailable (private mode,
 * opaque-origin sandbox), a successful unlock must STILL open the door for
...
/**
 * SHA-256 with a pure-JS fallback. The published app can be served through a
 * CSP-sandboxed flow whose document has an OPAQUE origin — not a secure
 * context, so `crypto.subtle` simply doesn't exist there and the door would
 * hang on "Checking…" forever. The fallback needs nothing from the platform.
 */
...
// Compact, dependency-free SHA-256 (FIPS 180-4). Verified against node:crypto
// in access.test.ts for every access code.
...
/** Test-only export: the pure-JS path, bypassing WebCrypto. */
export function sha256HexJsForTest(text: string): string {
  return sha256HexJs(new TextEncoder().encode(text));
}
...
export async function tryUnlock(code: string): Promise<AccessProfile | null> {
  ...
  sessionProfile = profile; // the door opens even when storage can't persist
  try {
    localStorage.setItem(KEY, JSON.stringify(profile));
  } catch {
    /* private mode / sandbox: session-only access */
  }
  notify();
  return profile;
}
